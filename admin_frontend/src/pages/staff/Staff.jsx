...
function Staff() {
  ...
  if(isError) return <div>Error: {error?.message || 'Failed to load staff'}</div>
  ...
  return (
    <>
      ...
      <div className="userList"> 
        <DataGrid rows={data || []} columns={columns} getRowId={(row) => row._id} style={{ fontSize:'18px',borderRadius:'1rem',backgroundColor :" #fff0f1"}}/>
      </div>
    </>
  );
}
