...
const UserLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.data) {
      ...
      res.data.user.type === 'ADMIN' ? navigate('/admin/dashboard'):  navigate('/dashboard') 
    ...
    const autoLogin = async () => {
      if (!access_token) {
        try {
          ...
          if (res.data) {
            storeToken(res.data.token)
            let { access_token: newToken } = getToken()
            dispatch(setUserToken({ access_token: newToken }))
            // Immediate redirect to dashboard
            navigate(res.data.user.type === 'ADMIN' ? '/admin/dashboard' : '/dashboard')
          }
        } catch (error) {
